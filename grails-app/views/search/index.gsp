...
<%@ page import="com.ml.analytics.Search" %>
<!DOCTYPE html>
<html>
	<head>
		...
		<div id="list-search" class="content scaffold-list" role="main">
			...
			<thead>
					<tr>
						...
						<g:sortableColumn property="name" title="${message(code: 'search.name.label', default: 'Name')}" />
				...
				<g:each in="${searchInstanceList}" status="i" var="searchInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${searchInstance.id}">${fieldValue(bean: searchInstance, field: "name")}</g:link></td>
			...
			<div class="pagination">
				<g:paginate total="${searchInstanceCount ?: 0}" />
			</div>
		</div>
	</body>
</html>
